...
export default async function ProjectDetailPage({
  ...
  return (
    <article className="pb-20">
      ...
      {/* Hero Banner */}
      <div className="relative h-[60vh] min-h-[500px] w-full bg-[#0A0A0A]">
        <img
          src={project.banner}
          alt={project.name}
        ...
        <div className="absolute bottom-0 inset-x-0">
          <div className="container mx-auto px-4 md:px-8 pb-12">
            <div className="max-w-4xl">
              <div className="inline-flex items-center space-x-2 rounded-full px-3 py-1 text-xs font-semibold backdrop-blur-md border mb-6 bg-white/10 text-white border-white/10">
                {project.status}
              </div>
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                {project.demoLink && (
                  <a href={project.demoLink} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-primary text-white font-semibold rounded-xl flex items-center hover:bg-primary/90 transition-colors">
                    <Globe className="w-5 h-5 mr-2" />
                    Live Demo
                  </a>
                )}
                {project.githubRepo && (
                  <a href={project.githubRepo} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-white/10 text-white font-semibold rounded-xl flex items-center border border-white/10 hover:bg-white/20 transition-colors">
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(project.features || ["Real-time data synchronization", "Secure authentication", "Responsive design", "Analytics dashboard"]).map((feature: string, idx: number) => (
