...
export default function EditProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formData, setFormData] = useState<ConnectProject>({
    id: "",
    name: "",
    description: "",
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        if (docSnap.exists()) {
          setFormData(docSnap.data() as ConnectProject);
        } else {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card border border-white/5 p-6 rounded-xl space-y-4">
                ...
                />
                {formData.banner && (
                  <div className="text-xs text-green-400 mt-2 break-all">Current banner URL: {formData.banner}</div>
                )}
              </div>
